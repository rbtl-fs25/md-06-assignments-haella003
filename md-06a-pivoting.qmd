...
2.  What's the `tidyverse` Package? Describe in maximum two sentences below.
    ...
    tidyverse contains different R packages to help organizing and handling data. It can help with tables (kable) or tyding up (tidyr).
...
typeof(levels_income_cat) #character
...
glimpse(waste_data_long_mean)
```

## Task 3: Table display

1.  Starting with the `waste_data_long_mean` object, execute the code and observe the output in the Console. Would you publish this table in this format in a report? no, the table should have the opposite shape: long down and not wide
...
2.  Execute the code and observe the output in the Console. Would you publish this table in a report in this format? no, it is still confusing and too wide
...
ggplot(data = waste_data_long_mean,
  ...
  scale_fill_manual(values = c("high income" = "lightblue", "upper-middle income" = "blue", "lower-middle income" = "lightgreen", "low income" = "darkgreen")) +
  scale_x_continuous(breaks = seq(0, 50, 5)) +
  #scale_fill_brewer(type = "qual", palette = 3) + #this rest is added from solution bc I didnt know. I added it to make it complete for render
  theme_minimal() + 
